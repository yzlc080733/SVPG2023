...
def get_arguments():
    ...
    parser.add_argument('--model', type=str, default='rwtaprob',
            choices=['mlp3soft', 'mlp3relu', 'rwtaprob', 'snnbptt'])
    ...
    # ---------------------------------------------------
    parser.add_argument('--train_num', type=int, default=2000)
...
if __name__ == "__main__":
    ...
    if args.model == 'mlp3soft':
        ...
        model = model_snnbptt.SNNBPTT3(
                ...
                entropy_ratio=args.entropy,)
    elif args.model == 'rwtaprob':              # working
        import model_rwta
    ...
    # -----main loop---------
    best_val_num = 0        # To reduce training time (early stop)
    ...
    for train_epi_i in range((last_train_epi_num + 1), args.train_num):
        ...
        if train_epi_i % args.val_freq == (args.val_freq - 1):
            ...
            if val_performance_mean == 525:
                best_val_num = best_val_num + 1
                if best_val_num >= 5:
                    break
            else:
                best_val_num = 0
