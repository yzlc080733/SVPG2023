...
def get_arguments():
    ...
    parser.add_argument('--task', type=str, default='cartpole',
                        choices=['mnist', 'vizdoom', 'gymip', 'cartpole'])
...
if __name__ == "__main__":
    ...
    if args.task in ['gymip', 'cartpole']:
    ...
    # Environment Setup
    if args.task == 'gymip':      # InvertedPendulum
        import env_gymip
    ...
    elif args.task == 'cartpole':      # CartPole
        import env_cartpole
        env = env_cartpole.CartPole(dev=torch_device)
        input_dimension, output_dimension = env.state_dimension, env.action_num
        mem = memory_lib.MemoryBuffer(s_size=input_dimension, a_size=output_dimension, dev=torch_device)
    else:
