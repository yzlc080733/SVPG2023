...
def get_arguments():
    ...
    parser.add_argument('--task', type=str, default='ai2thor',
            choices=['mnist', 'cifar10', 'vizdoom', 'gymip', 'gymdip', 'ai2thor'])
    ...
    parser.add_argument('--val_num', type=int, default=8)
...
if __name__ == "__main__":
    ...
    if args.task == 'mnist':
        ...
        train_batch_size, test_batch_size = 100, 10000
        mem = memory_lib.MemoryBatch()
    elif args.task == 'ai2thor':            ##################
        import env_ai2thor
        env = env_ai2thor.AI2THOR_NAVIGATION()
        if args.cuda < 0:
            env.convert_all_to_torch_cpu()
        else:
            env.convert_all_to_torch_gpu()
        ...
        import env_gymip
        env = env_gymip.GYMIP(train_xml_name=args.gymip_train_xml)
        if args.cuda < 0:
            env.convert_all_to_torch_cpu()
        else:
            env.convert_all_to_torch_gpu()
        ...
        train_batch_size, test_batch_size = 1, 1
        mem = memory_lib.MemoryList(gamma=args.gamma)
    elif args.task == 'gymdip':         # InvertedDoublePendulum
        import env_gymdip
        env = env_gymdip.GYMDIP()
        if args.cuda < 0:
            env.convert_all_to_torch_cpu()
        else:
            env.convert_all_to_torch_gpu()
        ...
        mem = memory_lib.MemoryList(gamma=args.gamma)
        # state value patch
        if args.model in ['mlp3relu', 'mlp3soft']:
            env.value_patch = -0.5          # [0, 1] -> [-0.5, 0.5]
    # -----model-------------
    ...
    for train_epi_i in range((last_train_epi_num + 1), args.train_num):
        ...
        for train_step_i in range(env.max_step_num):
            ...
            print('Input: ')
            action_chosen_onehot = torch.zeros([1, env.action_num]).cuda()
            while True:
                com = input()
                if com == 'w':
            ...
            if env.done_signal == True:
                break
        # -----Train-------------
        if args.task in ['gymip', 'gymdip', 'ai2thor']:  # 
